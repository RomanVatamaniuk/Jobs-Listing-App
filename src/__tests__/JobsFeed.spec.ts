...
type PaginationWrapper = VueWrapper<ComponentPublicInstance<PaginationProps, {}, {}, {}, {}, PaginationEmits>>;



const mockJobs: Job[] = [
  { id: 1, title: 'Job A' }, { id: 2, title: 'Job B' }, { id: 3, title: 'Job C' },
  { id: 4, title: 'Job D' }, { id: 5, title: 'Job E' }, { id: 6, title: 'Job F' },
  { id: 7, title: 'Job G' }, { id: 8, title: 'Job H' }, { id: 9, title: 'Job I' },
  { id: 10, title: 'Job J' },
  { id: 11, title: 'Job K' },
];
...
const getJobs = vi.fn<[], Promise<void>>(); // Explicitly type the spy function
...
describe('JobsFeed.vue', () => {
    ...
    wrapper = mount(JobsFeed, {
      global: {
        stubs: {
          JobCard: {
            props: ['job'],
            template: '<div data-testid="job-card">{{ job.title }} ({{ job.id }})</div>'
          },
          Pagination: {
            name: 'Pagination',
            props: ['currentPage', 'totalPages'],
            emits: ['update:currentPage'],
            template: '<div data-testid="pagination">Page: {{ currentPage }} / Total: {{ totalPages }}</div>'
          },
        },
      },
    });
  ...
  it('correctly calculates totalPages for empty and full data sets', async () => {
    const getPaginationStub = (): PaginationWrapper => wrapper.findComponent({ name: 'Pagination' }) as PaginationWrapper;
  ...
  it('updates paginatedJobs when currentPage changes (Pagination logic)', async () => {
    ...
    const paginationStub: PaginationWrapper = wrapper.find('[data-testid="pagination"]').findComponent({ name: 'Pagination' }) as PaginationWrapper;
